...
const getIssueIcon = (category: string): React.ReactNode => {
  ...
  if (cat.includes("trash") || cat.includes("garbag") || cat.includes("waste"))
    return <TrashIcon />;
  if (
    cat.includes("sewage") ||
    cat.includes("drain") ||
...
const MyReportsScreen = () => {
  ...
  const loadIssues = async () => {
    ...
    try {
      ...
      const myIssues = allIssues.filter((i: any) => i.userId === user?.id);
      setIssues(myIssues);
    } catch (err) {
      console.error("Failed to fetch my reports:", err);
      setError("Failed to load your reports. Please check your connection.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <Text style={styles.issueCount}>{filteredIssues.length} issues</Text>
        </View>
      </View>
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            {activeTab === "All"
              ...
              : `You have no ${activeTab.toLowerCase()} issues`}
          </Text>
        </View>
      ) : (
        <FlatList
...
const styles = StyleSheet.create({
  ...
  issueCount: { fontSize: 12, fontWeight: "600", color: "#64748B" },
